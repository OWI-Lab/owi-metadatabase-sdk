...
from owi.metadatabase._utils.utils import (
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "value_1",
            "value_2",
            (False, "Values of value_1 and value_2 are different."),
        ),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "test",
            1.0,
            (False, "Types of test and 1.0 are different: str and float."),
        ),
...
_safe_text = st.text().filter(lambda value: value.lower() != "nan")
_safe_float = st.floats(allow_nan=False, allow_infinity=False, width=32)
_simple_values = st.one_of(st.integers(), _safe_float, _safe_text, st.none())


@given(value=_simple_values)
def test_fix_nan_is_idempotent(value):
    result = fix_nan(value)
    result_2 = fix_nan(result)
    comp = deepcompare(result, result_2)
    assert comp[0], comp[1]


@given(values=st.lists(_simple_values, max_size=10))
def test_fix_nan_list_idempotent(values):
    result = fix_nan(values)
    result_2 = fix_nan(result)
    comp = deepcompare(result, result_2)
    assert comp[0], comp[1]
