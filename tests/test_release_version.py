...
def test_apply_version_uses_bumpversion_configuration(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    package_version = tmp_path / "src/owi/metadatabase/_version.py"
    test_imports = tmp_path / "tests/test_imports.py"
    test_namespace = tmp_path / "tests/test_namespace.py"
    installation_doc = tmp_path / "docs/getting-started/installation.md"
    uv_lock = tmp_path / "uv.lock"

    package_version.parent.mkdir(parents=True, exist_ok=True)
    test_imports.parent.mkdir(parents=True, exist_ok=True)
    installation_doc.parent.mkdir(parents=True, exist_ok=True)
    ...
    monkeypatch.setattr(release_version, "PACKAGE_VERSION_PATH", package_version)
    monkeypatch.setattr(release_version, "TEST_IMPORTS_PATH", test_imports)
    monkeypatch.setattr(release_version, "TEST_NAMESPACE_PATH", test_namespace)
    monkeypatch.setattr(release_version, "INSTALLATION_DOC_PATH", installation_doc)
    ...
    assert changed_paths == [
        ...
        "GitVersion.yml",
        "pyproject.toml",
        "src/owi/metadatabase/_version.py",
